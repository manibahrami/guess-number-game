...
number_to_guess = 0 tries_count = 0

def start_new_game(): global number_to_guess, tries_count # Declaring intent to modify global variables number_to_guess = random.randint(1, 100) # Selects a new random number tries_count = 0 # Resets the try counter
...
def check_guess(): global tries_count
...
--- Main Window Setup ---
main_window = tk.Tk() # Creates the main window main_window.title("Guessing Game") # Sets the window title main_window.geometry("300x250") main_window.resizable(False, False) # Prevents the user from resizing the window

--- Widgets (elements inside the window) ---
Instruction Label
instruction_label = tk.Label(main_window, text="Guess a number between 1 and 100:", font=("Arial", 10)) instruction_label.pack(pady=5)

Input box for user's guess
guess_input_box = tk.Entry(main_window, width=20, font=("Arial", 12)) guess_input_box.pack(pady=5) guess_input_box.focus_set() guess_input_box.bind("", lambda event: check_guess()) # Checks guess when Enter is pressed

Guess Button
guess_button = tk.Button(main_window, text="Guess", command=check_guess, font=("Arial", 10)) guess_button.pack(pady=5)

Attempts Label
attempts_label = tk.Label(main_window, text="Tries: 0", font=("Arial", 10), fg="purple") attempts_label.pack(pady=5)

Result Label
result_label = tk.Label(main_window, text="", font=("Arial", 10), fg="blue") result_label.pack(pady=5)

Button for new game
new_game_button = tk.Button(main_window, text="New Game", command=start_new_game, font=("Arial", 10)) new_game_button.pack(pady=10)
